...
class Game:
    ...
    def __init__(self, player_type):
        ...
        self.permanentAgentRL = ag.Qlearning(
            self.screen_height, self.bar_height, 0.85, 0.99)
        if self.player_type == 'agentRL':
            self.agentRL = ag.Qlearning(
                self.screen_height, self.bar_height, 0.85, 0.99)
        ...
        self.opponent = pygame.Rect(self.bar_width, (self.screen_height / 2) - (
            self.bar_height / 2), self.bar_width, self.bar_height)
    ...
    def play(self):
        ...
        while True:
            ...
            s = self.permanentAgentRL.centre_to_state(
                self.opponent.centery, self.screen_height, self.bar_height)
            self.opponent.y = self.permanentAgentRL.update(
                s, self.opponent, self.ball, self.screen_height, self.ball_speed_x, True)
            if self.player_type == 'human':
                self.animation_human()
            elif self.player_type == 'agentAI':
                self.animation_agentAi()
            elif self.player_type == 'agentRL':
                s = self.agentRL.centre_to_state(
                    self.player.centery, self.screen_height, self.bar_height)
                self.player.y = self.agentRL.update(
                    s, self.player, self.ball, self.screen_height, self.ball_speed_x, False)
            ...
            player_txt = self.game_font.render(
                f"{self.player_score}", False, self.light_grey)
            self.screen.blit(player_txt, (600, 470))
            opponent_txt = self.game_font.render(
                f"{self.opponent_score}", False, self.light_grey)
